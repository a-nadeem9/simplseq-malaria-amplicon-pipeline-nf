...
parser$add_argument("--randomize", type="integer",
                    help="Use randomized read order during error learning: 1 yes, 0 no (default: 1)")
parser$add_argument("--multithread", type="integer",
                    help="Use DADA2 multithreading: 1 yes, 0 no (default: 1)")
parser$add_argument("--seed", type="integer",
                    help="Optional random seed for deterministic/replay runs")
...
if (!is.null(args$randomize) && args$randomize != '') {
  randomize <- as.logical(as.numeric(args$randomize))
}
if (!is.null(args$multithread) && args$multithread != '') {
  multithread <- as.logical(as.numeric(args$multithread))
}
if (!is.null(args$seed) && args$seed != '') {
...
dir.create(file.path(work_dir, "filtered"), showWarnings = FALSE, recursive = TRUE)
...
if (filter == TRUE) {
	...
	out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, truncQ=c(0,0), maxN=0, rm.phix=TRUE,
            compress=TRUE, multithread=multithread, verbose=TRUE, matchIDs=matchIDs)
}
...
errF <- learnErrors(filtFs, multithread=multithread, verbose=2, randomize=randomize, MAX_CONSIST=max_consist)
print("starting error model learning for reverse reads...")
errR <- learnErrors(filtRs, multithread=multithread, verbose=2, randomize=randomize, MAX_CONSIST=max_consist)
...
dadaFs <- dada(derepFs, err=errF, selfConsist=selfConsist, multithread=multithread, verbose=TRUE, OMEGA_A=omega_a)
print("starting dada2 for reverse reads...")
dadaRs <- dada(derepRs, err=errR, selfConsist=selfConsist, multithread=multithread, verbose=TRUE, OMEGA_A=omega_a)
...
write.table(seqtab, file=output_filename, quote = FALSE, sep = "\t", col.names = NA)
